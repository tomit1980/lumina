...
describe("editMessage — author-only, no permission check at all", () => {
  it("the author can edit their own message even after their role loses message.send", async () => {
...
describe("deleteMessage — author or message.deleteAny", () => {
  it("the author can delete their own message without any special permission", async () => {
...
describe("toggleReaction — gated by conversation visibility only", () => {
  it("a viewer-only member of a private channel can react", async () => {
...
describe("sendToUser find-or-creates the DM atomically", () => {
  it("a second call between the same two users reuses the existing DM instead of creating a new one", async () => {
    ...
    const second = await run(() => result.current.sendToUser("u_priya", "second message"));
...
describe("markChannelRead / getUnreadCount — per-user bookkeeping", () => {
  ...
  it("markChannelRead resets the reader's own unread count without affecting other users'", async () => {
    ...
    await run(() => result.current.switchUser("u_maya"));
    await run(() => result.current.markChannelRead("c_engineering"));
